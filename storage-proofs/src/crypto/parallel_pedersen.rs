...
impl ParallelPedersen {
    ...
    pub fn new() -> Self {
        // Load the first exp-table partition so that worker will have in immediately available.
        /*
        let exp_table_path = JJ_PARAMS.exp_table_path().as_ref().unwrap();
        let exp_table = read_exp_table_range(0, N_SEGMENTS_PER_PARTITION, exp_table_path);
        */
        let exp_table = JJ_PARAMS.pedersen_hash_exp_table().to_vec();
        let n_segments_read = exp_table.len();
        let exp_table = Arc::new(RwLock::new(exp_table));
        let exp_table_range = Arc::new(RwLock::new(ExpTableRange::new(0, n_segments_read)));
