...
use crate::stacked::{
    ...
    hash::hash3,
    params::{
        get_node, Labels, PersistentAux, Proof, PublicInputs, PublicParams, ReplicaColumnProof,
        Tau, TemporaryAux, TransformedLayers, Tree, WindowProof, WrapperProof,
    },
...
impl<'a, H: 'static + Hasher, G: 'static + Hasher> StackedDrg<'a, H, G> {
    ...
    fn encode_all_windows(
        ...
        let labels: Vec<Mutex<DiskStore<_>>> = (0..layers - 1)
            .map(|_| -> Result<_> {
                let r = Mutex::new(DiskStore::new(layer_size)?);
                Ok(r)
            })
            .collect::<Result<_>>()?;
...
mod tests {
    ...
    fn test_extract_node<H: 'static + Hasher>() {
        ...
        for node in 0..nodes {
            ...
            let decoded_node =
                StackedDrg::<H, Blake2sHasher>::extract(&pp, &replica_id, &data_copy, node)
                    .expect("failed to extract data");
